...
class RandomPlacementP(DataPlacement):
    def placement(self):
...
class StagedStripeMergingForRanP(StagedStripeMerging):
...
if __name__ == '__main__':
    ...
    g = 2
    x = np.prod(x_list)
    ran = RandomPlacementP(encoding_type, x, k, l, g)
    ran.placement()
    ran.print_placement_res()
    staged_merge = StagedStripeMergingForRanP(len(x_list), x_list, k, l, g, ran)
