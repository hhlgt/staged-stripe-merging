...
bool ECProject::random_generate_kv(std::string &key, std::string &value,
...
bool ECProject::random_generate_value(std::string &value, int value_length)
{
  /*生成一个固定大小的随机value*/
...
std::string ECProject::gen_key(int key_len, std::unordered_set<std::string> keys)
{
  std::random_device rd;
  std::mt19937 gen(rd());
  std::uniform_int_distribution<unsigned int> dis(0, 25);
